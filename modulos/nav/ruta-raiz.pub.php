...
<?
$sql="select cat_id, cat_nombre, cat_ruta_amigable, cat_publicar from  categoria	ORDER BY cat_id ASC";
$rs = $query -> consulta($sql);
$num = $query -> num_registros($rs);
if($num>0){
for($i=0;$i<$num;$i++){
  list($mod_id,$mod_nombre,$mod_url,$mod_publicar)=$query->obt_fila($rs);
  if ($mod_id!=1){
?>
